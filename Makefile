...
devenv: .venv requirements-dev.txt ## create a python virtual environment with tools to dev, run and tests cookie-cutter
...
# INTEGRATION -----------------------------------
docker-compose-meta.yml: metadata/metadata.yml
...
build build-devel build-kit build-kit-devel build-x build-devel-x: docker-compose-build.yml docker-compose-meta.yml ## builds images, -devel in development mode, -kit using docker buildkit, -x using docker buildX (if installed)
...
version-service-patch version-service-minor version-service-major: versioning/service.cfg ## kernel/service versioning as patch
	...
	$(MAKE) --no-print-directory docker-compose-meta.yml

.PHONY: push push-force push-version push-latest pull-latest pull-version tag-latest tag-version publish-local
...
.PHONY: tag-local
tag-local:
	docker tag $(DOCKER_REGISTRY)/$(DOCKER_IMAGE_NAME):$(DOCKER_IMAGE_TAG) local/$(DOCKER_IMAGE_NAME):production


version_valid = $(shell test $$(echo $(DOCKER_IMAGE_TAG) | cut -f 1 -d .) -gt 0 > /dev/null && echo "image version is valid")
...
shell shell-devel: $(COMPOSE_INPUT_DIR) $(COMPOSE_OUTPUT_DIR) .env down ## Starts a shell instead of running the container. Useful for development.
